...
try:
    ...
    from osocio.utils.scheduling import guardar_programacion, cargar_programacion, limpiar_programacion
    from osocio.interface.field_validation_ui import build_field_validation_tab
    from osocio.interface.dealer_comparator_ui import build_dealer_comparator_tab
    from osocio.interface.ids_dinamicos_ui import abrir_popup_ids_dinamicos
...
def _ensure_serialized_setup():
    if _SETUP_PATCHED[0]:
        return
    try:
        from osocio.core import base_form_filler as _bff
...
def iniciar_interfaz(autostart_leads=False):
    ...
    def _refresh_win_task_lbl():
        try:
            from osocio.utils.win_task_scheduler import listar
            tareas = listar()
        except Exception:
            tareas = []
        if tareas:
    ...
    def cmd_registrar_win_task():
        ...
        if not messagebox.askyesno(
            ...
            return
        try:
            from osocio.utils.win_task_scheduler import registrar
    ...
    def cmd_quitar_win_task():
        if not messagebox.askyesno("Programador de Windows",
            ...
            return
        try:
            from osocio.utils.win_task_scheduler import desregistrar
            ok, msg = desregistrar()
        except Exception as e:
            ok, msg = False, str(e)
    ...
    def _on_toggle_abrir_app():
        ...
        _save_ui_prefs()
        try:
            from osocio.utils.win_task_scheduler import listar, registrar
            if not listar():
    ...
    def execute_send_leads(scheduled=False, retry_only=None):
        """Delegado a osocio/interface/envio_leads.py.

        El cuerpo real vive alla; aca se arma el contexto con lo que necesita del
        ...
        ctx = SimpleNamespace(
            ...
            _forzar_foreground=_forzar_foreground,
            _restore_from_tray=_restore_from_tray,
            active_p_tab=active_p_tab,
            btn_enviar=btn_enviar,
            btn_retry_leads=btn_retry_leads,
            email_entry=email_entry,
            excel_mode_holder=excel_mode_holder,
            excels_mode=excels_mode,
            log_message=log_message,
            mercados_mode=mercados_mode,
            paises_list=paises_list,
            refresh_execute_state=refresh_execute_state,
            root=root,
            scheduler_cfg_leads=scheduler_cfg_leads,
            selected_countries=selected_countries,
            selected_disp=selected_disp,
            url_max_var=url_max_var,
            var_adjuntar_res=var_adjuntar_res,
            var_adjuntar_ss=var_adjuntar_ss,
            var_enviar_email=var_enviar_email,
            var_modo_email=var_modo_email,
            var_pausar_autenticacion=var_pausar_autenticacion,
        ...
        return ejecutar_envio_leads(ctx, scheduled=scheduled, retry_only=retry_only)
    ...
    def run_sync_action():
        ...
        def sync_worker():
            try:
                from osocio.core.massive_check_runner import sincronizar_reporte_con_matriz
        ...
        def worker():
            try:
                ...
                if var_enviar_email.get() and success and not stop_event.is_set():
                    ...
                    from osocio.interface.helpers_interface import enviar_email_revision_masiva
...
# TAB 3: GENERAR EXCELS CON DATOS (excel)
    ...
    try:
        from osocio.utils.data_generator import DOC_TYPES_BY_COUNTRY as _DOC_TYPES
    except Exception:
    ...
    def _rows_for_pais(pais, pares, columnas, device=None, doc_types=None):
        try:
            from osocio.utils.data_generator import fixed_values_for_url
        except Exception:
    ...
    def _force_close():
        try:
            from osocio.core.browser_manager import kill_active_drivers
            kill_active_drivers()
        except Exception:
            pass
        try:
