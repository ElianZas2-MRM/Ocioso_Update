...
Mapa de alto nivel — quién dispara qué y a dónde van los resultados:

![Arquitectura de la app](docs/screenshots/00_arquitectura.png)

- **Punto de entrada** (`run.py`): el único arranque. Según cómo lo llames, abre la **interfaz gráfica**, arranca el **programador autónomo**, o corre un país puntual por consola (`--run-country`) sin abrir ninguna ventana.
- **Interfaz gráfica** (`osocio/interface/`): la ventana con las pestañas de este README. Una pestaña por archivo; `main_interface.py` tiene la ventana principal y el scheduler.
- **Programador autónomo** (`osocio/autonomous_runner.py`): corre en background según el calendario semanal, sin que nadie abra la app.
- **Motor de formularios** (`osocio/core/`): el corazón de la automatización. `BaseFormFiller` abre el navegador, detecta el formulario paso a paso, lo completa y lo envía. Está partido por responsabilidad en `osocio/core/form_filler/` (navegación, casillas, desplegables, IDs dinámicos, reglas por mercado, formularios AEM).
- **LambdaTest** (`osocio/providers/`): cuando el envío incluye Mac o Android, corre sobre dispositivos reales en la nube en vez de un navegador local.
- **Validación & QA** (`osocio/validation/`, `osocio/core/dealer_comparator_runner.py`): la pestaña de Validación de Campos y el Comparador de Dealers. Abren el navegador para chequear reglas o dealers **sin llegar a enviar un lead real**.
- **Configuración** (`data/` + `json/`): los datos de prueba y el mapeo de campos por país que el motor necesita para saber qué escribir y dónde.
- **Resultados** (`resultados/`): el Excel de resultados, las capturas de cada lead y el mail final.

> Para el detalle técnico —el recorrido de una corrida, cómo se resuelven las rutas, cómo se prueba el motor sin navegador— ver [`docs/arquitectura.md`](docs/arquitectura.md).
...
7. **💳 FORMA DE COMPRA** (opcional, **solo Argentina** — la card aparece únicamente con ese mercado seleccionado) — tildá **"Tiene ¿Cómo deseas realizar la compra?"** si el form tiene ese campo (id `buy_pda`, ej. `.../gm_front/form/pda-raq`) y elegí con cuál de las dos opciones se chequean los dealers: **Compra al contado/Financiación** o **Plan de ahorro**.
   - **Por qué es un campo aparte del de Modelo**: no es un campo más del form, **gatea toda la cascada**. Hasta que no se elige una opción, `region`, `city` y `dealer` vienen con `disabled=true` (región ya trae sus opciones en el DOM; city y dealer, solo el placeholder). Y al elegirla, el form **repuebla** ciudades y dealers, porque las listas **son distintas entre una opción y la otra** (ej. en CABA, `LINIERS` y su dealer solo existen con *Plan de ahorro*).
   - Por eso el comparador lo selecciona **primero de todo**: antes que el selector de Modelo y antes de avanzar los pasos del form. Aplica **con o sin** selector de Modelo tildado (si están los dos, va este primero).
   - Como los dealers cambian según la opción, **hacé una corrida por cada una**, combinándolo con la columna de filtro del Excel (ej. `PDA` = `Sí` con *Plan de ahorro*, `VENTA 0KM` = `Sí` con *Compra al contado/Financiación*).
   - Si tildaste la opción y el form no tiene el campo —o no tiene la opción elegida— ese form da **FAIL** completo con el motivo, en vez de compararse mal en silencio contra la cascada trabada (mismo criterio que el selector de Modelo).
8. **📦 MODO DE SALIDA** — **"Solo Excel"** o **"Excel + Capturas"**. Con capturas, quedan como PNG sueltos **junto al Excel dentro de la misma carpeta** del form (sin ZIP; lo comprimís vos si querés).
9. **Envío por email** — no hay configuración de mail dentro de esta pestaña: se controla **solo desde la barra superior de la app** (campo **Email destinatario** + checkbox **Enviar mail**), igual que para el resto de las pestañas. Si al ejecutar el Comparador tenés "Enviar mail" tildado, al terminar **cada form** se manda un mail con su reporte: adjunta la **carpeta completa (Excel + capturas) en un ZIP** si elegiste "Excel + Capturas", o **solo el Excel** si elegiste "Solo Excel". El cuerpo trae el resumen PASS/FAIL/EXTRA/DUPLICADO/OCULTO/NOTA.
...
10. **💾 CONFIGURACIONES GUARDADAS** — guardá el mapeo completo con un nombre y reusalo (incluye el toggle y la opción de **Forma de compra**):
...
Todo el código cuelga de un único paquete `osocio/`:

```
run.py                     punto de entrada unico
osocio/
  core/                    motor de llenado de formularios
    form_filler/           BaseFormFiller partida en mixins por responsabilidad
    browser/               acciones sobre el navegador
  interface/               la UI, una pestaña por archivo
  validation/              motor de la pestaña Validación de Campos
  providers/               LambdaTest Mac y Android
  utils/                   rutas, datos, drivers, programación, scroll dinámico
  forms/                   runner compartido de los países
  autonomous_runner.py     el programador que corre en background
tests/                     503 tests
docs/                      documentación
Asset/                     iconos e imágenes que se empaquetan en el .exe
```

Las que la app crea y llena sola (no se versionan):

- **`data/`** — Excels de entrada por país. Los generás desde la pestaña "Generar Excels con Datos".
- **`json/`** — configuración persistente. Ver [la sección de abajo](#qué-hay-adentro-de-json-no-la-borres).
- **`drivers/`** — drivers de navegador. Los descarga y actualiza la app sola.
- **`temporales/`** — logs y respaldos de tus Excels antes de modificarlos.
- **`resultados/`** — **todo** lo que la app produce, en subcarpetas:

  ```
  resultados/
    screenshots_<Pais>/        capturas del envío de leads
    resultados_<Pais>.xlsx     el Excel de resultados
    lambdatest_mac/            corridas de LambdaTest Mac
    lambdatest_android/        corridas de LambdaTest Android
    dealers/                   reportes del Comparador de Dealers
    resultado_urlsinsertas/    reportes de Revisión Masiva
  ```
...
Los drivers viven en `drivers/` (junto al proyecto, o junto al `.exe` al compilar), pero **no se versionan en el repo ni se reemplazan a mano**: los gestiona `osocio/utils/driver_updater.py`.
...
`osocio/providers/lambdatest_mac/` y `osocio/providers/lambdatest_android/` van empaquetados por PyInstaller **adentro del `.exe`**, no como carpetas sueltas al lado — no hace falta copiarlos a mano.
