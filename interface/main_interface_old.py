...
def _excel_tiene_filas(ruta):
    """True si el Excel existe y tiene al menos una fila de datos (no vacía).
    Se usa para NO arrancar una corrida (local o LambdaTest) con un Excel
    inexistente o sin leads."""
    try:
        ...
        # Si no se pudo leer (archivo abierto/corrupto), no bloquear por esto.
        return True
...
def _recargar_credenciales(*_):
    global lt_username_var, lt_key_var
    if lt_username_var is not None and lt_key_var is not None:
        u, ak = _leer_creds_txt()
        lt_username_var.set(u)
        lt_key_var.set(ak)
...
def _init_overlay_countries(paises_keys):
    ...
    for w in pf.winfo_children():
        w.destroy()
    # Estilos de barra (crear una sola vez)
    try:
        _st = ttk.Style()
        for _sname, _color in (("OvAmber", "#f59e0b"), ("OvGreen", "#10b981"), ("OvRed", "#ef4444")):
            _st.configure(f"{_sname}.Horizontal.TProgressbar",
                          troughcolor="#2a1a44", bordercolor="#2a1a44",
                          background=_color, lightcolor=_color, darkcolor=_color)
    except Exception:
        pass
    for key in paises_keys:
        ...
        bar = ttk.Progressbar(row, orient="horizontal", length=130, mode="determinate",
                              maximum=100, value=0, style="OvAmber.Horizontal.TProgressbar")
        ...
        _run_state["progress_widgets"][key] = {"status_lbl": status_lbl, "count_lbl": count_lbl, "bar": bar}
...
def _update_progress(key, done=None, total=None, ok=None, status=None):
    ...
    def _apply():
        ...
        def _set_bar(style, value):
            if not bar:
                return
            try:
                bar.configure(style=f"{style}.Horizontal.TProgressbar", value=value)
            except Exception:
                pass
        ...
        if s == "waiting":
            ...
            w["count_lbl"].configure(text=f"{d}/{t} leads" if t else "...")
            _set_bar("OvAmber", (d / t * 100) if t else 0)
        elif s == "done_ok":
            w["status_lbl"].configure(text="✓ Completado", fg="#10b981")
            w["count_lbl"].configure(text=f"{t}/{t} leads" if t else "Completado")
            _set_bar("OvGreen", 100)
        elif s == "done_error":
            w["status_lbl"].configure(text="✗ Con errores", fg="#ef4444")
            _det = f"{o}/{t} OK" if t else "con errores"
            w["count_lbl"].configure(text=_det)
            _set_bar("OvRed", 100)
        elif s == "stopped":
...
def _build_generar_excels_tab(parent):
    ...
    def _obtener_sufijos_y_nombres_excel(pais):
        sufijos_excels = []
        if chrome_var.get():
            sufijos_excels.append(("Chrome", f"Lead_information_Formulario_{pais}_Chrome.xlsx"))
        if firefox_var.get():
            sufijos_excels.append(("Firefox", f"Lead_information_Formulario_{pais}_Firefox.xlsx"))
        if edge_var.get():
            sufijos_excels.append(("Edge", f"Lead_information_Formulario_{pais}_Edge.xlsx"))
        if lt_mac_var.get():
            sufijos_excels.append(("Mac", f"Lead_information_Formulario_{pais}_Mac.xlsx"))
        if lt_android_var.get():
            sufijos_excels.append(("Android", f"Lead_information_Formulario_{pais}_Android.xlsx"))
            
        return sufijos_excels
    ...
    def _generar():
        ...
        sufijos_excels = _obtener_sufijos_y_nombres_excel(pais)
        if not sufijos_excels:
            status_var.set("Seleccioná al menos un dispositivo/navegador.")
            status_lbl.config(fg="#f4a261")
            messagebox.showwarning("Selección vacía", "Seleccioná al menos un dispositivo/navegador en los checkboxes.")
            return
        
        any_exists = False
        existing_names = []
        for dev_suffix, excel_nombre in sufijos_excels:
            excel_ruta = os.path.join(DATA_DIR, excel_nombre)
            if os.path.exists(excel_ruta):
                any_exists = True
                existing_names.append(excel_nombre)
                
        if any_exists:
            reemplazar = messagebox.askyesno(
                "Excel existente",
                f"Ya existen los siguientes Excels para {pais}:\n" + "\n".join(existing_names) + "\n\n¿Reemplazarlos con nuevos datos?"
            )
            if not reemplazar:
        ...
        try:
            for dev_suffix, excel_nombre in sufijos_excels:
                ...
                for landing_url, form_url in pares:
                    _form_counter += 1
                    datos = generar_fila_datos(pais, device=dev_suffix)
                    if pais == "Brasil":
            ...
            status_var.set(f"✓ Fila(s) generada(s) para {len(sufijos_excels)} dispositivo(s)")
            status_lbl.config(fg="#a8e6a3")
            btn_abrir_carpeta.pack(anchor="w", pady=(6, 0))
        except PermissionError:
            status_var.set("Cerrá los archivos Excel y volvé a intentar.")
            status_lbl.config(fg="#f4a261")
        except Exception as exc:
    ...
    def _limpiar():
        ...
        if not any_exists:
            status_var.set("No existen archivos para limpiar.")
            status_lbl.config(fg="#f4a261")
            return
            
        if not messagebox.askyesno("Limpiar Excels", f"¿Borrar todas las filas de datos de los excels seleccionados para {pais}?\n\nArchivos:\n" + "\n".join(existing_names)):
            return
        ...
        try:
            for dev_suffix, excel_nombre in sufijos_excels:
                ...
                if os.path.exists(excel_ruta):
                    ...
                    cleaned_files.append(excel_nombre)
            if cleaned_files:
                status_var.set(f"✓ Datos limpiados de: {', '.join(cleaned_files)}")
                status_lbl.config(fg="#a8e6a3")
        except PermissionError:
            status_var.set("Cerrá los archivos Excel antes de limpiar.")
            status_lbl.config(fg="#f4a261")
    ...
    def _generar_con_urls_existentes():
        ...
        # Buscar un archivo origen para leer las URLs
        source_path = None
        source_name = None
        
        # Intentar primero con el Main, luego con los específicos
        candidates = [f"Lead_information_Formulario_{pais}_Main.xlsx"] + [name for _, name in sufijos_excels]
        ...
        try:
            df_existente = pd.read_excel(source_path, dtype=str, keep_default_na=False)
        except Exception as exc:
            status_var.set(f"Error al leer el Excel {source_name}: {exc}")
            status_lbl.config(fg="#f4a261")
            return
        ...
        ok = messagebox.askyesno(
            ...
            f"manteniendo las URLs leídas de {source_name}.\n\n"
            f"¿Continuar?"
        )
        if not ok:
            return
        ...
        try:
            ...
            status_var.set(f"✓ {n_filas} fila(s) regeneradas para {len(sufijos_excels)} dispositivo(s)")
            status_lbl.config(fg="#a8e6a3")
            btn_abrir_carpeta.pack(anchor="w", pady=(6, 0))
        except PermissionError:
            status_var.set("Cerrá los archivos Excel y volvé a intentar.")
...
def _build_lambdatest_tab(parent):
    ...
    def _on_inner_tab_changed(event):
        _recargar_credenciales()
    ...
    def _ejecutar_lt_mac():
        ...
        # VALIDAR ARCHIVOS EXCEL ANTES DE INICIAR
        for pais in paises_sel:
            excel_nombre = f"Lead_information_Formulario_{pais}_Mac.xlsx"
            excel_ruta = os.path.join(DATA_DIR, excel_nombre)
            if not _excel_tiene_filas(excel_ruta):
                mostrar_alerta_excel_faltante(pais, "Mac", excel_nombre)
                return
        ...
        summaries_mac = []
        _email_modo = (cargar_config_global().get("ui_prefs", {}) or {}).get("email_modo", "por_pais")
        ...
        def _run_pais_mac(pais):
            ...
            try:
                ...
                summary = lt_controller.run(
                    ...
                    stop_event=stop_ev,
                    build_name=f"Osocio LT Mac - Envío Manual - {pais}",
                )
                ok = summary.get("ok", 0)
                ...
                if summary.get("error"):
                    ...
                    _update_progress(pais, done=total, total=total, ok=ok,
                                     status="done_ok" if (failed == 0 and ok >= total) else "done_error")
                _entry = {
                    "pais": pais,
                ...
                with remaining_lock:
                    summaries_mac.append(_entry)
                if _email_modo == "por_pais" and not stop_ev.is_set():
                    from interface.helpers_interface import enviar_email_resultados_consolidados
                    threading.Thread(target=lambda e=_entry: enviar_email_resultados_consolidados([e]),
                                     daemon=True).start()
            except Exception as e:
                ...
                with remaining_lock:
                    ...
                    if remaining[0] == 0:
                        _set_running(False)
                        if _email_modo == "consolidado" and summaries_mac and not stop_ev.is_set():
                            from interface.helpers_interface import enviar_email_resultados_consolidados
                            threading.Thread(
    ...
    def _ejecutar_lt_android():
        ...
        # VALIDAR ARCHIVOS EXCEL ANTES DE INICIAR
        for pais in paises_sel:
            excel_nombre = f"Lead_information_Formulario_{pais}_Android.xlsx"
            excel_ruta = os.path.join(DATA_DIR, excel_nombre)
            if not _excel_tiene_filas(excel_ruta):
                mostrar_alerta_excel_faltante(pais, "Android", excel_nombre)
                return
        ...
        def _run_pais_android(pais):
            ...
            try:
                ...
                summary = lt_android_controller.run(
                    ...
                    stop_event=stop_ev,
                    build_name=f"Osocio LT Android - Envío Manual - {pais}",
                )
                ok = summary.get("ok", 0)
                total = summary.get("total", 0)
                if summary.get("error"):
                ...
                with remaining_lock:
                    summaries_android.append(_entry)
                if _email_modo == "por_pais" and not stop_ev.is_set():
                    from interface.helpers_interface import enviar_email_resultados_consolidados
                    threading.Thread(target=lambda e=_entry: enviar_email_resultados_consolidados([e]),
                                     daemon=True).start()
                ...
                with remaining_lock:
                    ...
                    if remaining[0] == 0:
                        _set_running(False)
                        if _email_modo == "consolidado" and summaries_android and not stop_ev.is_set():
                            from interface.helpers_interface import enviar_email_resultados_consolidados
                            threading.Thread(
...
def iniciar_interfaz():
    ...
    global lt_mac_var, lt_android_var, visible_browser_var, email_modo_var, minimizar_a_bandeja_var
    ...
    # Si el usuario lo tilda, la X minimiza a la bandeja del sistema (persistente).
    minimizar_a_bandeja_var = BooleanVar(value=bool(_ui_prefs.get("minimizar_a_bandeja", False)))
    ...
    _u_init, _ak_init = _leer_creds_txt()
    lt_username_var = StringVar(value=_u_init)
    lt_key_var = StringVar(value=_ak_init)
    ...
    # Opt-in: los adjuntos NO se envían salvo que el usuario tilde el checkbox en esta sesión.
    cfg_global["adjuntar_resultados"] = False
    cfg_global["adjuntar_screenshots"] = False
    ...
    adjuntar_resultados_var = BooleanVar(value=False)   # opt-in explícito por sesión
    adjuntar_screenshots_var = BooleanVar(value=False)  # opt-in explícito por sesión
    ...
    Checkbutton(
        ...
        fg="#85C1E9",
        activebackground=APP_BG_COLOR,
        activeforeground="#85C1E9",
        selectcolor=APP_BG_COLOR,
    ).pack(side=LEFT, padx=5)

    Checkbutton(
        frame_navegador,
        text="LambdaTest Android",
    ...
    # Comportamiento del botón cerrar (X) de la ventana principal
    frame_cierre = Frame(frame_izquierda, bg=APP_BG_COLOR)
    frame_cierre.pack(anchor="w", pady=2)
    Label(
        frame_cierre,
        text="Cerrar:",
        bg=APP_BG_COLOR,
    ...
    Checkbutton(
        frame_cierre,
        text="Minimizar a la bandeja al cerrar (en vez de cerrar el programa)",
        variable=minimizar_a_bandeja_var,
        bg=APP_BG_COLOR,
        fg="white",
        activebackground=APP_BG_COLOR,
        activeforeground="white",
    ...
    def _persistir_opciones_email(*_):
        ...
        cfg["ui_prefs"] = {
            ...
            "minimizar_a_bandeja": bool(minimizar_a_bandeja_var.get()),
        }
    ...
    for _var in (enviar_mail_var, adjuntar_resultados_var, adjuntar_screenshots_var,
                 ...
                 desktop_var, mobile_var, visible_browser_var, email_modo_var, minimizar_a_bandeja_var):
        _var.trace_add("write", _persistir_opciones_email)
    _persistir_opciones_email()

    def _on_device_check_changed(*_):
        if lt_mac_var.get() or lt_android_var.get():
            _recargar_credenciales()
    lt_mac_var.trace_add("write", _on_device_check_changed)
    lt_android_var.trace_add("write", _on_device_check_changed)
    ...
    def mostrar_alerta_excel_faltante(pais, dispositivo, excel_nombre):
        alert_win = Toplevel(root)
        alert_win.title("⚠️ Excel no encontrado")
        alert_win.geometry("520x220")
        alert_win.resizable(False, False)
        alert_win.configure(bg=APP_BG_COLOR)
        alert_win.transient(root)
        alert_win.grab_set()
        
        # Centrar la ventana en pantalla
        alert_win.update_idletasks()
        x = root.winfo_x() + (root.winfo_width() - alert_win.winfo_width()) // 2
        y = root.winfo_y() + (root.winfo_height() - alert_win.winfo_height()) // 2
        alert_win.geometry(f"+{x}+{y}")
        
        # Icono y Texto
        Label(
            alert_win,
            text="⚠️ ARCHIVO EXCEL NO GENERADO",
            font=("Segoe UI", 12, "bold"),
            bg=APP_BG_COLOR,
            fg="#ff6b6b",
        ).pack(pady=(20, 10))
        
        msg = f"No existe (o está vacío / sin leads) el Excel para {pais} en el dispositivo '{dispositivo}':\n\n{excel_nombre}"
        Label(
            alert_win,
            text=msg,
            font=("Segoe UI", 10),
        ...
        frame_btns = Frame(alert_win, bg=APP_BG_COLOR)
        frame_btns.pack(fill="x", side="bottom", pady=20)
        
        def ir_a_generar():
            alert_win.destroy()
            try:
                for tab_id in app_notebook.tabs():
                    if app_notebook.tab(tab_id, "text") == "Generar Excels con Datos":
        ...
        btn_generar = ttk.Button(
            frame_btns,
            text="Ir a generar Excel para este dispositivo",
            command=ir_a_generar,
            style="Section.TButton"
        )
        btn_generar.pack(side=LEFT, padx=(30, 5), expand=True)
        ...
        btn_cerrar.pack(side=RIGHT, padx=(5, 30), expand=True)
    ...
    def _ejecutar_envio():
        ...
        navegadores = [b for b, v in [
            ("chrome", chrome_var), 
            ("firefox", firefox_var), 
            ("edge", edge_var),
            ("lambdatest_mac", lt_mac_var),
            ("lambdatest_android", lt_android_var)
        ] if v.get()]
        
        has_local = any(b in ("chrome", "firefox", "edge") for b in navegadores)
        viewports = [vp for vp, v in [("fullscreen", desktop_var), ("600x738", mobile_var)] if v.get()]
        
        if not navegadores:
            messagebox.showwarning("Configuración", "Seleccioná al menos un navegador / dispositivo.")
            return

        if has_local and not viewports:
            messagebox.showwarning("Configuración", "Seleccioná al menos un viewport para los navegadores locales.")
            return
        ...
        for pais in paises_sel:
            for nav in navegadores:
                if nav == "lambdatest_mac":
                    ...
                    dev_name = "Chrome" if nav == "chrome" else ("Firefox" if nav == "firefox" else "Edge")
                excel_nombre = f"Lead_information_Formulario_{pais}_{dev_name}.xlsx"
                excel_ruta = os.path.join(DATA_DIR, excel_nombre)
                if not _excel_tiene_filas(excel_ruta):
                    mostrar_alerta_excel_faltante(pais, dev_name, excel_nombre)
                    return

        if "lambdatest_mac" in navegadores or "lambdatest_android" in navegadores:
            cfg_actual = cargar_config_global()
            cfg_actual["lambdatest"] = {
                "username": lt_username_var.get().strip(),
                "access_key": lt_key_var.get().strip(),
        ...
        def _run_combo(pais, nav, vp):
            ...
            try:
                if nav == "lambdatest_mac":
                    ...
                    failed = summary.get("failed", 0)
                    if summary.get("error"):
                        print(f"✗ {pais} (Mac): {summary['error']}")
                        _update_progress(pais, total=total, ok=ok, status="stopped" if stop_ev.is_set() else "done_error")
                    else:
                        print(f"✓ Leads de {pais} enviados (Mac) ({ok}/{total})")
                        _update_progress(pais, done=total, total=total, ok=ok, status="done_ok" if (failed == 0 and ok >= total) else "done_error")
                    if _consolidado and summary.get("results_excel"):
                    ...
                    import sys as _sys
                    _lt_dir = os.path.join(BASE_DIR, "lambdatest_android")
                    if _lt_dir not in _sys.path:
                        _sys.path.insert(0, _lt_dir)
                    import lt_android_controller  # type: ignore[import]
                    dev_name = lt_android_device_var.get() if 'lt_android_device_var' in globals() or 'lt_android_device_var' in locals() else "Galaxy S23"
                    with_ss = lt_android_screenshots_var.get() if 'lt_android_screenshots_var' in globals() or 'lt_android_screenshots_var' in locals() else False
                    summary = lt_android_controller.run(
                        pais=pais,
                        device_name=dev_name,
                        with_screenshots=with_ss,
                        log_fn=_make_lt_log(pais),
                        stop_event=stop_ev,
                        build_name=f"Osocio LT Android - Envío Manual - {pais}",
                    )
                    ...
                    failed = summary.get("failed", 0)
                    if summary.get("error"):
                        print(f"✗ {pais} (Android): {summary['error']}")
                        _update_progress(pais, total=total, ok=ok, status="stopped" if stop_ev.is_set() else "done_error")
                    else:
                        print(f"✓ Leads de {pais} enviados (Android) ({ok}/{total})")
                        _update_progress(pais, done=total, total=total, ok=ok, status="done_ok" if (failed == 0 and ok >= total) else "done_error")
                    if _consolidado and summary.get("results_excel"):
                        with _resultados_lock:
                            _resultados_consolidado.append({
                ...
                print(f"Error ejecutando {pais} ({nav}/{vp if vp else ''}): {exc}")

        def _run_pais_con_browsers(pais):
            _update_progress(pais, status="running")
            browser_combos = []
            for nav in navegadores:
                if nav in ("lambdatest_mac", "lambdatest_android"):
                    browser_combos.append((nav, None))
                else:
                    for vp in viewports:
                        browser_combos.append((nav, vp))
    ...
    for pais in paises:
        ...
        for i, script in enumerate(pais["scripts"], start=1):
            ...
            version_var = StringVar(value="Chrome")

            def get_excel_filename_by_version(version_val):
                return f"Lead_information_Formulario_{pais['nombre']}_{version_val}"

            btn_crear_abrir = ttk.Button(
                frame_botones_derecha,
                text="Abrir Excel" if os.path.exists(excel_ruta) else "Crear Excel",
                style="Section.TButton",
            )
            btn_crear_abrir.pack(side=RIGHT, padx=5)

            # Combobox para seleccionar la versión del Excel
            cb_version = ttk.Combobox(
                frame_botones_izquierda,
                textvariable=version_var,
                values=["Chrome", "Firefox", "Edge", "Mac", "Android"],
                state="readonly",
                width=10,
            )
            cb_version.pack(side=LEFT, padx=5)

            def crear_y_actualizar_excel_dynamic(name, script_idx, frame_parent, tree, editor):
                crear_y_actualizar_excel(name, script_idx, frame_parent, None, None, tree, editor)
                on_excel_version_change()

            def on_excel_version_change(*_):
                selected_ver = version_var.get()
                current_excel_nombre = get_excel_filename_by_version(selected_ver)
                current_excel_ruta = os.path.join(DATA_DIR, f"{current_excel_nombre}.xlsx")

                btn_actualizar_tabla.config(command=lambda n=current_excel_nombre, tw=tree_excel, ce=cell_editor: cargar_excel_a_tabla(n, tw, ce))
                def _guardar_con_edit_dynamic(n=current_excel_nombre, tw=tree_excel, ce=cell_editor):
                    ce.finish_edit()
                    guardar_desde_tabla(n, tw)
                btn_guardar_tabla.config(command=_guardar_con_edit_dynamic)

                if os.path.exists(current_excel_ruta):
                    btn_crear_abrir.config(text="Abrir Excel", command=lambda n=current_excel_nombre: abrir_excel(n))
                    cargar_excel_a_tabla(current_excel_nombre, tree_excel, cell_editor)
                else:
                    btn_crear_abrir.config(
                        text="Crear Excel",
                        command=lambda n=current_excel_nombre, f=1, fp=frame_pais, tw=tree_excel, ce=cell_editor: crear_y_actualizar_excel_dynamic(n, f, fp, tw, ce)
                    )
                    tree_excel.delete(*tree_excel.get_children())
                    tree_excel["columns"] = ["info"]
                    tree_excel["show"] = "headings"
                    tree_excel.heading("info", text="Información")
                    tree_excel.column("info", width=400)
                    tree_excel.insert("", "end", values=[f"No existe el Excel para '{selected_ver}'. Créelo con el botón 'Crear Excel'."])

            cb_version.bind("<<ComboboxSelected>>", on_excel_version_change)

            # Configurar los comandos iniciales
            ...
            # Inicializar enlaces de botones y tabla
            on_excel_version_change()
            
            _excel_reloaders.append((lambda v=version_var: get_excel_filename_by_version(v.get()), tree_excel, cell_editor))

    #ttk.Separator(root, orient="horizontal", style="App.TSeparator").pack(fill="x", pady=10)

    def _on_leads_tab_activated(event):
        selected = app_notebook.select()
        tab_text = app_notebook.tab(selected, "text").strip()
        if tab_text == "Envio de Leads":
            for _n, _tw, _ce in _excel_reloaders:
                name = _n() if callable(_n) else _n
                if os.path.exists(os.path.join(DATA_DIR, f"{name}.xlsx")):
                    cargar_excel_a_tabla(name, _tw, _ce)
        elif tab_text == "LambdaTest":
            _recargar_credenciales()
    ...
    def _on_close():
        global _tray_instance
        # Si hay una ejecución en curso (modal/overlay abierto), no cerrar: minimizar
        # para que la ejecución continúe. El usuario puede restaurar la ventana luego.
        if _run_state.get("running"):
            try:
                ...
                pass
            return
        if os.name == 'nt' and minimizar_a_bandeja_var.get():
            root.withdraw()
